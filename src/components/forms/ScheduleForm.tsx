...
export default function ScheduleForm({
    ...
    const form = useForm<z.infer<typeof scheduleFormSchema>>({
        ...
        defaultValues: {
            timezone: schedule?.timezone ?? Intl.DateTimeFormat().
    ...
    const groupAvailabilityFields = Object.groupBy(availabilityFields.map((field, index) => (
        { ...field, index }
                    ...
                    {DAYS_OF_WEEK_IN_ORDER.map(dayOfWeek => (
                        <Fragment key={dayOfWeek}>
                            ...
                            <div className="flex flex-col gap-3">
                                ...
                                {groupAvailabilityFields[dayOfWeek]?.map((field, labelIndex) => (
                                    <div className="flex flex-col gap-1" key={field.id}>
                                        ...
                                        <FormMessage>
                                            {form.formState.errors.availabilities?.at?.(
                                                field.index)?.root?.message
                                            }
                                        </FormMessage>
                                        <FormMessage>
                                            {form.formState.errors.availabilities?.at?.(
                                                field.index)?.startTime?.message
                                            }
                                        </FormMessage>
                                        <FormMessage>
                                            {form.formState.errors.availabilities?.at?.(
                                                field.index)?.endTime?.message
                                            }
                                        </FormMessage>
                                    </div>
                                ))}
