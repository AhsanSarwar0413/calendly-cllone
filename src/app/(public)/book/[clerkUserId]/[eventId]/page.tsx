...
export default async function BookEventPage({
    ...
    const event = await db.query.EventTable.findFirst({
        where: ({ clerkUserId: userIdCol, isActive, id }, { and, eq }) => and(eq(isActive, true), eq(userIdCol, clerkUserId), eq(id, eventId)),
    })
    ...
    const validTimeSlotsForUser = await getValidTimesFromSchedule(eachMinuteOfInterval({
        start: startDate,
        end: endDate
    }, {
        step: 15
    ...
    if (validTimeSlotsForUser.length == 0) {
    ...
    return (
        <Card className="max-w-4xl mx-auto">
            ...
            <CardContent>
                <MeetingForm
                    validTimes={validTimeSlotsForUser}
                    eventId={event.id}
